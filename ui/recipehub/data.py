...
from .cluster_utils import post_recipe
...
def update_recipe(recipe_id, ingredients, steps, message=None, title=None, description=None):
    from .utils import get_detailed_recipe
    ret = request('put', '/recipe/{}/'.format(recipe_id), {
         'recipe_id': recipe_id,
         'ingredients': ingredients,
         'steps': steps,
         'title': title,
...
def fork_recipe(user_id, recipe_id):
    from .utils import get_detailed_recipe
    ret = request('post', '/fork/{}/'.format(recipe_id), {
         'user_id': user_id
    })
    post_recipe(ret)
    return ret
